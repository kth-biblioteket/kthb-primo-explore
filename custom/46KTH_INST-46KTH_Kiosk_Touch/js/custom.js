...
(function () {
	...
	*****************************************/
	...
	app.controller('prmExploreMainAfterController', function ($scope, $translate) {
        var vm = this;
	...
	***************************************/
	...
	app.component('prmStackMapAfter', {
		bindings: {parentCtrl: '<'},
		controller: 'prmStackMapAfterController'
	});
	
	app.controller('prmStackMapAfterController', function ($scope, $http, $translate) {
		var vm = this;
		angular.element(document).ready(function() {
			var links = document.getElementsByTagName("a");
			for (var i = 0, l = links.length; i < l; i++) {
	...
	**********************************************************/
	app.component('prmSearchResultAvailabilityLineAfter', {
		...
		template: `<div ng-if="$ctrl.onlinelink">
						<div>
							<span class="">
								<span>No access to online material from this search computer</span>
							</span>
						</div>
					</div>
					<ul ng-if="$ctrl.show">
