...
(function () {
	...
	*****************************************/
	...
	app.controller('prmExploreMainAfterController', function ($compile, $scope, $http,$rootScope,$timeout,$templateCache, $element,Idle,$location,$translate) {
			...
			'<md-toolbar>' +
				'<md-fab-actions class="md-toolbar-tools zero-padding buttons-group">' +
					...
					'<div class="kth_fabaction">' +
						...
						'<a class="button-with-icon kth-favorite" aria-label="DB" target="_blank" href="{{$ctrl.kth_databaseurl}}">' +
		...
		*************************************************************/
		...
		'<div tabindex="-1" ng-click="$event.stopPropagation()" aria-live="assertive" ng-if="::(!$ctrl.isFavorites  && $ctrl.showBookmark())" class="pin-button">' +
			'<md-button ng-if="!$ctrl.showPin()" ng-class="{\'pinned\':!$ctrl.showPin()}" ng-disabled="$ctrl.isFavoritesDisabled()" class="md-icon-button custom-button pin-button" aria-label="{{::(\'nui.aria.favorites.pin\' | translate:\'{index: \\\'\'+$ctrl.index +\'\\\'}\')}}" ng-click="$event.stopPropagation(); $event.preventDefault(); $ctrl.updateFavorites($event);">' +
				...
				//'<prm-icon aria-label="{{::(\'nui.aria.favorites.pin\' | translate:\'{index: \\\'\'+ $ctrl.index+\'\\\'}\')}}" [icon-type]="::$ctrl.actionsIcons.pin.type" [svg-icon-set]="::$ctrl.actionsIcons.pin.iconSet" [icon-definition]="::$ctrl.actionsIcons.pin.icon"></prm-icon>' +
			'</md-button>' +
			'<md-button ng-if="$ctrl.showPin()" ng-class="{\'unpinned\':$ctrl.showPin()}" class="md-icon-button custom-button unpin-button" aria-label="{{::(\'nui.aria.favorites.unpin\' | translate:\'{index: \\\'\'+ $ctrl.index+\'\\\'}\')}}" ng-click="$event.stopPropagation();$event.preventDefault();$ctrl.updateFavorites($event);">' +
				'<md-tooltip md-delay="400">' +
					'<span translate="{{::(\'nui.favorites.remove.tooltip\' | translate:\'{index: \\\'\'+ $ctrl.index+\'\\\'}\')}}"></span>' +
				'</md-tooltip>' +
				//KTHB hjärta
				...
				//'<prm-icon aria-label="{{::(\'nui.aria.favorites.unpin\' | translate:\'{index: \\\'\'+ $ctrl.index+\'\\\'}\')}}" [icon-type]="::$ctrl.actionsIcons.unPin.type" [svg-icon-set]="::$ctrl.actionsIcons.unPin.iconSet" [icon-definition]="::$ctrl.actionsIcons.unPin.icon">' +
				'</prm-icon>' +
			'</md-button>' +
			...
			'</div>' +
		'</div>' +
		'<md-button ng-if="::($ctrl.isFavorites  && $ctrl.showBookmark())" class="md-icon-button custom-button prm-primary" aria-label="{{::(\'nui.aria.favorites.unpin\' | translate:\'{index: \\\'\'+($ctrl.index)+\'\\\'}\')}}" ng-click="$event.stopPropagation();$event.preventDefault();$ctrl.unpinFavorites();">' +
			'<md-tooltip md-delay="400">{{::("nui.favorites.remove.tooltip" | translate)}}</md-tooltip>' +
			'<prm-icon aria-label="{{::(\'nui.aria.favorites.unpin\' | translate:\'{index: \\\'\'+($ctrl.index)+\'\\\'}\')}}" class="" [icon-type]="::$ctrl.actionsIcons.unPin.type" [svg-icon-set]="::$ctrl.actionsIcons.unPin.iconSet" [icon-definition]="::$ctrl.actionsIcons.unPin.icon"></prm-icon>' +
		'</md-button>' +
		'<prm-save-to-favorites-button-after parent-ctrl="$ctrl"></prm-save-to-favorites-button-after>'
		);
		/*
		$templateCache.put('components/search/briefResult/briefResultContainer.html',
		'<div ng-click="$ctrl.handleDetails($ctrl.item,$event, true)" role="listitem" class="list-item-primary-content result-item-primary-content" ng-class="::{\'has-checkbox\': $ctrl.isfavorites, \'new-result-item\' : ($ctrl.index && $ctrl.newMetalibItem())}" data-recordid="{{::$ctrl.recordId}}" id="SEARCH_RESULT_RECORDID_{{::$ctrl.recordId}}" layout="row">
		<span class="list-item-count" ng-if="!$ctrl.isFullView">
	...
	//'</div>'
		...
		'<md-card class="default-card zero-margin">' +
			...
			'<md-card-content>' +
				...
				'<ul>' +
					...
					'<li translate>nui.noresults.suggestions4</li>' +
					'<li translate>nui.noresults.suggestions5</li>' +
				'</ul>' +
			'</md-card-content>' +
		'</md-card>' +
		...
		*******************/
		...
		//'<div class="layout-row">' + 
			'<div class="full-view-container" layout-xs="column" layout-sm="column" layout-gt-sm="row" id="full-view-container" flex>' + 
				'<div class="full-view-inner-container" flex>' + 
					...
					'<div class="full-view-section" ng-repeat="service in $ctrl.services.slice(1) track by $index" ng-attr-id="{{service.scrollId}}" ng-if="!service.isDisabled && service.scrollId!=\'citationTrails\' && service.scrollId!=\'tags\'">' + 
		...
		'<div ng-if="$ctrl.wostimesCited!==\'\'">' +
			'<span class="wostimesCited">{{$ctrl.wostimesCited}}</span> <span translate="{{\'nui.citation_trail.link.citedExternal\'}}"></span><a target="_new" href="{{$ctrl.wossourceURL}}"> Web of Science&trade;</a>' +
		'</div>' +
		//scopus citations
		...
		'<div tabindex="-1" class="primo-scrollbar" 1sticky offset="24" sticky-class="is-stuck" disabled-sticky="!$ctrl.mediaQueries.gtsm" ng-if="$ctrl.showFacetSection() " ng-class="$ctrl.activeMultipleFacets ? \'multifacets-active\' : \'\'">' +
			'<div tabindex="-1" class="sidebar-inner-wrapper" layout="column">' +
				...
				*/
				...
				//'<div style="display:flex" tabindex="-1" ng-if="$ctrl.totalResults > 1 || $ctrl.isFiltered()" class="sidebar-section margin-top-small compensate-padding-left">' +
	...
	*****************************************/
	app.component('prmLogoAfter', {
			...
			template: '<div><a href="{{$ctrl.parentCtrl.kthb_link}}"><span class="kth-sitenameheader" translate="nui.header.sitename"></span></a></div>'
	});
